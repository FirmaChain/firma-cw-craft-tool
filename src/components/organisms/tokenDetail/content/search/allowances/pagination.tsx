                  ...
                  header === "Receiver" ?
                    <TableCell width={'740px'} color={'#00827A'} key={cellIndex}>
                      <CellWrapper>
                        ...
                        <CellColorTypo color={'#00827A'} onClick={(e: any) => onClickAddress(e, item[header])}>{shortenAddress(item[header], 11, 11)}</CellColorTypo>
                        <CopyIconButton text={item[header]} width={'22px'} height={'22px'} />
                      </CellWrapper>
                    </TableCell> :
                    header === "Amount" ?
                      <TableCell width={'264px'} color={'#DCDCDC'} key={cellIndex}>{getTokenStrFromUTokenStr(item[header], decimals)}</TableCell> :
                      <TableCell color={'#DCDCDC'} key={cellIndex}>{parseExpires(JSON.stringify(item[header]))}</TableCell>
                ))}
              </tr>
            ))}
          </tbody>
