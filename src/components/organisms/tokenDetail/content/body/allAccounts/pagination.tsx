...
const PaginatedTable = ({ decimals, headers, data, itemsPerPage }: IProps) => {
  ...
  return (
    <TableContainer
      style={{
        width: 'calc(100% - 48px)', height: '100%', padding: '20px 24px',
        borderRadius: '8px',
    ...
    >
            ...
            {displayedData.map((item, rowIndex) => (
              <tr key={rowIndex}>
                {headers.map((header, cellIndex) => (
                  ...
                  <TableCell color={header === 'Wallet Address' ? '#00827A' :'#DCDCDC'} key={cellIndex}>
                    {header === 'Wallet Address' ? 
                      <CellWrapper>
                        <Icons.wallet width={'22px'} height={'22px'} fill={'#00827A'} />
                        <CellColorTypo color={'#00827A'} onClick={(e: any) => onClickAddress(e, item[header])}>{shortenAddress(item[header], 8, 10)}</CellColorTypo>
                        <CopyIconButton text={item[header]} width={'22px'} height={'22px'} />
                      </CellWrapper>
                      :
                      getTokenStrFromUTokenStr(item[header], decimals.toString())
                    }
                  </TableCell>
                ))}
              </tr>
      ...
      {displayedData.length !== 0 && <PaginationContainer style={{ justifyContent: 'flex-end', alignItems: 'center', alignContent: 'center' }}>
        <PaginationButton
