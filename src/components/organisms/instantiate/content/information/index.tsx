...
import { IconBackground, InformationBody, InformationWrapper, TextGroupWrapper, TitleDescription, TitleText, TitleWrapper, TokenNameSymbol } from "./style";
...
const Information = ({
  ...
  return (
    <InformationWrapper>
      ...
      <InformationBody>
        <TokenNameSymbol>
          <InputTextWithLabel label={"Token Name"} placeHolderLeft={"ex) My CW Token"} enableLength={true} maxLength={30} value={tokenName} onChange={handleTokenName} />
          <InputTextWithLabel label={"Token Symbol"} placeHolderLeft={"ex) MCT, FCT"} enableLength={true} maxLength={6} value={tokenSymbol} onChange={handleTokenSymbol} />
        </TokenNameSymbol>
        {!isBasic ?
          <>
            <InputTextWithLabel label={"Decimals"} placeHolderLeft={"0 ~ 16"} maxLength={16} type={'number'} value={decimals} onChange={handleDecimals} />
            <InputTextWithLabel label={"Label"} placeHolderLeft={"ex) Event reward contract"} value={label} onChange={handleLabel} />
          </>
          : null
        }
        <InputUrlWithImage label={isBasic ? "Token Image Link (Optional)" : "Marketing Logo (Optional)"} placeHolder={"ex) https://example.thisismy.token.jpg"} value={tokenLogoUrl} onChange={handleTokenLogoUrl} />
        <InputTextWithLabel label={isBasic ? "Token Description (Optional)" : "Marketing Description (Optional)"} placeHolderLeft={"ex) This is my token"} enableLength={false} value={tokenDescription} onChange={handleDescription} />
        {!isBasic ?
          <>
            <InputTextWithLabel label={"Marketing Address (Optional)"} placeHolderLeft={"Input wallet Address"} value={marketingAddress} onChange={handleMarketingAddress} />
            <InputTextWithLabel label={"Marketing Project (Optional)"} placeHolderLeft={"ex) http://firmachain.org"} value={marketingProject} onChange={handleMarketingProject} />
          </>
          : null
        }
      </InformationBody>
